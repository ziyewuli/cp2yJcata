...
var area= {
		...
		change:function(v){
			var that=area,sels=that.selectsId,dsy=that.areaTree(),opt0=that.opt0,rate=[0];
			for(i=0;i<v;i++){ 
				rate.push(document.getElementById(sels[i]).selectedIndex-1);
			};
			var nextSel=document.getElementById(sels[v]),_rate=rate.join('_');
			with(nextSel){
				length = 0;
				options[0]=new Option(opt0[v],opt0[v]);
				if(v && document.getElementById(sels[v-1]).selectedIndex>0 || !v){
					if(dsy.Exists(_rate)){
						var place=dsy.Items[_rate],len=place.length;
						for(i=0;i<len;i++){
							options[length]=new Option(place[i],place[i]);  
						}//end for
						...
						if(!!def){
							if(v==0&&def.length>=1){
							   document.getElementById(sels[0]).options[def[0]].selected=true;
							}
							if(that.firstLoad&&v==1&&def.length>=2){
								document.getElementById(sels[1]).options[def[1]].selected=true;
								that.firstLoad=false;
							}
						}
					}
				} //end if v
				if(++v<sels.length){ that.change(v); } 
			}//End with
			for(i=0;i<sels.length;i++){
				if(document.getElementById(sels[i]).selectedIndex-1==-1){
					$('#'+sels[i]).css('color','#999');
				}else{
					$('#'+sels[i]).css('color','#333');
				}
			};
		},
		selectsId:[],//"s_province","s_city","s_county"
		//opt0:["省份","地级市","区/县级市"],
		opt0:["请选择","请选择","请选择"],
		firstLoad:true,
		def:[],
		init:function(o){
			...
			for(var key in o){
			 ...
			 $('#county').change(function(){
				 ...
				 index==0?$('#county').css('color','#999'):$('#county').css('color','#333');
			 })
			window.change=area.change;
			var sels=this.selectsId,len=sels.length-1,def=this.def; 
			for(i=0;i<len;i++){
			  document.getElementById(sels[i]).onchange= new Function("change("+(i+1)+")");
			}

			this.change(0);
			if(!!def&&def.length>=3){ 
				document.getElementById(sels[2]).options[def[2]].selected=true;
			}
		},
		getArea:function(){
			var sels=this.selectsId,len=sels.length;
			var address=[],index=[];
			for(i=0;i<len;i++){
				var obj=document.getElementById(sels[i]),_index=obj.selectedIndex;
				if(_index==0){
					return false;
				}else{ 
					index.push(_index);
					address.push(obj.options[obj.selectedIndex].text); 
				}
			}	
			return address.join(' ')+'#'+index.join(' ')+'#012'
